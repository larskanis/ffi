...
import static org.jruby.org.objectweb.asm.Opcodes.ACC_FINAL;
        ...
        for (int i = 0; i < signature.getParameterCount(); i++) {
            ...
            switch (parameterType) {
                ...
                case TRANSIENT_STRING:
                    ...
                    switch (parameterType) {
                        ...
                        case TRANSIENT_STRING:
                            ...
                            mv.invokestatic(p(JITRuntime.class),
                                    ...
                                    sig(ffi.MemoryIO.class, IRubyObject.class, ThreadContext.class, CachingCallSite.class));
                            break;


                        default:
                            // First try fast lookup based solely on what java type the parameter is
                            mv.invokestatic(p(JITRuntime.class), "lookupPointerMemoryIO", sig(ffi.MemoryIO.class, IRubyObject.class));
                            ...
                            mv.aload(0);
                            mv.getfield(p(JITNativeInvoker.class), builder.getParameterCallSiteName(i), ci(CachingCallSite.class));
                            mv.invokestatic(p(JITRuntime.class), "convertToPointerMemoryIO", sig(ffi.MemoryIO.class, ThreadContext.class, IRubyObject.class, CachingCallSite.class));
                            break;
                    }
                    ...
                    mv.aload(nextMemoryVar);
                    mv.invokevirtual(p(ffi.MemoryIO.class), "isDirect", sig(boolean.class));
                    ...
                    mv.label(direct);
                    mv.aload(nextMemoryVar);
                    mv.invokevirtual(p(ffi.MemoryIO.class), "address", sig(long.class));
            ...
            for (int i = 0, ptrIdx = 0; i < signature.getParameterCount(); i++) {
                switch (signature.getParameterType(i)) {
                    ...
                    case TRANSIENT_STRING:
                        ...
                        mv.dup();
                        mv.invokestatic(p(JITRuntime.class), "getMemoryIOStrategy", sig(PointerParameterStrategy.class, ffi.MemoryIO.class));
                        mv.aload(0);
