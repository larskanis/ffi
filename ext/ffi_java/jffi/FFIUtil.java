...
import ffi.CallbackInfo;
import ffi.NativeType;
import ffi.StructLayout;
import ffi.Type;
...
public final class FFIUtil {
    ...
    private static com.kenai.jffi.Type cacheFFIType(Type type) {
        ...
        synchronized (type) {
            ...
            } else if (type instanceof ffi.StructLayout) {

                ffiType = FFIUtil.newStruct((ffi.StructLayout) type);

            } else if (type instanceof ffi.StructByValue) {

                ffiType = FFIUtil.newStruct(((ffi.StructByValue) type).getStructLayout());

            } else if (type instanceof ffi.Type.Array) {

                ffiType = FFIUtil.newArray((ffi.Type.Array) type);

            } else if (type instanceof ffi.MappedType) {

                ffiType = FFIUtil.getFFIType(((ffi.MappedType) type).getRealType());
     ...
     */
    static final com.kenai.jffi.Aggregate newStruct(ffi.StructLayout layout) {
     ...
     * @return A new Struct descriptor.
     */
    static com.kenai.jffi.Array newArray(ffi.Type.Array arrayType) {
